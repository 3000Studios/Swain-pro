...
export default function HeroCanvas3D() {
  ...
  useEffect(() => {
    ...
    // Lights
    scene.add(new THREE.AmbientLight(0x00d4ff, 0.1))
    const l1 = new THREE.PointLight(0x00d4ff, 5, 150)
    l1.position.set(30, 35, 25)
    scene.add(l1)
    const l2 = new THREE.PointLight(0x8b5cf6, 5, 150)
    l2.position.set(-30, -25, 20)
    scene.add(l2)
    const l3 = new THREE.PointLight(0x10b981, 3, 100)
    ...
    // Orbiting rings
    const RING_DATA = [
      { r: 11, color: 0x00d4ff, ox: 0, oz: 0.4 },
      { r: 16, color: 0x8b5cf6, ox: 0.6, oz: 0.2 },
      { r: 20, color: 0x10b981, ox: 1.1, oz: 0.8 },
    ]
    ...
    const shards = Array.from({ length: SHARD_COUNT }, () => {
      ...
      const col = [0x00d4ff, 0x8b5cf6, 0x10b981][Math.floor(Math.random() * 3)]
    ...
    const starMat = new THREE.PointsMaterial({ color: 0xffffff, size: 0.25, transparent: true, opacity: 0.55 })
    scene.add(new THREE.Points(starGeo, starMat))

    // Grid floor
    const grid = new THREE.GridHelper(300, 60, 0x00d4ff, 0x0d0f17)
